...
if(($type == "jpg") || ($type == "jpeg")){
    ...
    if($size < 512000){
        ...
        if($uploaded == true){
            $ready = array("Archivo Cargado Correctamente", $narchive);
            echo json_encode($ready);
        }else {
            $ready = array("Error al cargar archivo", $narchive);
            echo json_encode($ready);
        }
    }else{
        echo "Error! - Archivo demasiado pesado";
    }
}else {
    echo "Error! -  Tipo de arhivo no soportado";
}
